...
class MusicPlayer:
    ...
    def __init__(self, music_dir="music", pl_dir="playlists", window_mode=False):
        pygame.init()
        # Try real audio first; fall back to dummy (silent) if no device available.
        # This happens over SSH where ALSA has no output device (error 524).
        try:
            pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=2048)
            self._audio_ok = True
        except pygame.error as e:
            print(f"[audio] No audio device ({e}) — running silent.")
